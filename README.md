<!-- HEADER -->
<h1 align="center">👋 Hi, I’m Mohammad Shahnawaz.</h1>
<p align="center">
  <img src="https://readme-typing-svg.demolab.com/?lines=--+probably+debugging+something+right+now&center=true&width=500&height=50&duration=3000&pause=1000">
</p>
<p align="center">
  🎓 Computer Science Engineering Student | 🤖 Machine Learning Enthusiast | 💻 Developer
</p>

<p align="center">
  📍 CMR Engineering College | 📊 CGPA: 8.47 <br>
  📧 shanu.sufiyan.2805@gmail.com <br>
  🔗 <a href="#">LinkedIn</a> | 💻 <a href="#">GitHub</a>
</p>

---

## 🚀 Professional Summary  
Motivated and detail-oriented third-year Computer Science Engineering student with a strong academic record. Passionate about **Machine Learning, Data Analytics, and Web Development**, with hands-on experience in building predictive ML models and responsive websites.  

Strong problem-solving mindset developed through hackathons, certifications, and technical events. Actively seeking opportunities to apply skills to real-world impactful projects.

---

## 🛠️ Tech Stack  

### 💻 Programming  
![Java](https://img.shields.io/badge/Java-orange?style=for-the-badge&logo=java)  
![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge&logo=python)  
![C](https://img.shields.io/badge/C-grey?style=for-the-badge&logo=c)  

- AI Model Development  
- Prompt Engineering  

### 🗄️ Database  
![MySQL](https://img.shields.io/badge/MySQL-blue?style=for-the-badge&logo=mysql)  
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-darkblue?style=for-the-badge&logo=postgresql)  

### 🌐 Web Development  
![HTML](https://img.shields.io/badge/HTML-red?style=for-the-badge&logo=html5)  
![CSS](https://img.shields.io/badge/CSS-blue?style=for-the-badge&logo=css3)  
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=for-the-badge&logo=javascript)  
![React](https://img.shields.io/badge/React-black?style=for-the-badge&logo=react)  

### 🤖 Machine Learning  
- Logistic Regression  
- Random Forest  
- XGBoost  
- Feature Engineering  

### ⚙️ Tools  
![Git](https://img.shields.io/badge/Git-orange?style=for-the-badge&logo=git)  
![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)  

### 📊 Data Analysis  
- Data Visualization  
- Dashboards  
- Model Evaluation  

---

## 📂 Projects  

### 😊 Human Emotion Recognition System  
- Built a deep learning model for facial emotion detection  
- Used CNN with image processing techniques  
- Detected emotions: Happy, Sad, Angry, Surprise, Fear, Neutral  
- Applied preprocessing & feature engineering for accuracy improvement  
- Evaluated using accuracy & confusion matrix  

**Tech:** Python, OpenCV, TensorFlow/Keras  

---

### 🏙️ Civic Pulse — Smart Civic Issue Platform  
- Platform for reporting and tracking civic issues  
- Bridges gap between citizens & government  
- Supports reporting issues in homes, roads, streets  
- Database-integrated complaint management system  

**Tech:** HTML, CSS, JavaScript, DBMS  

---

### 🏥 Sahayak — AI Health Chatbot 🥈  
- Built during hackathon (2nd Prize Winner)  
- AI chatbot for basic healthcare guidance  
- Designed conversational logic for user interaction  
- Provides instant responses to symptoms  

**Tech:** Python, NLP  

---

### 🛍️ Krish's Handicraft Store  
- Responsive frontend e-commerce website  
- Clean UI with interactive product features  
- Mobile-first design approach  

**Tech:** HTML, CSS, JavaScript  

---

## 🚧 Ongoing Project  

### 🎙️ Voice Command Assistant  
- Voice-controlled system automation tool  
- Executes commands like opening apps & web search  
- Uses speech recognition & text-to-speech  
- Focused on accuracy & real-time execution  

**Tech:** Python, SpeechRecognition, Pyttsx3  

---

## 🏆 Achievements & Certifications

### 📜 Professional Certifications
| Organization | Certificate | Verification |
| :--- | :--- | :--- |
| **Microsoft** | AI SKILLS PASSPORT | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/AI%20Skills%20Passport.pdf) |
| **Salesforce** | SALESFORCE AGENTFORCE SPECIALIST | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Salesforce%20AgentForce%20Specialist.pdf) |
| **Spoken Tutorial** | RDBMS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/RDMS.pdf) |
| **Spoken Tutorial** | PERL | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/PERL.pdf) |
| **Deloitte** | CYBER JOB SIMULATION | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/CYBER%20JOB%20SIMULATION.pdf) |
| **Hexart.in** | AI FOUNDATION | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/HEXART.AI%20AI%20FOUNDATION.pdf) |
| **Infosys** | CYBER SECURITY | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Infosys%20Cyber%20Security.pdf) |
| **Infosys** | NETWORK FUNDAMENTALS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Infosys%20Network%20Fundamentals.pdf) |
| **MongoDB** | MONGODB SKILLS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/MongoDB%20Skill.pdf) |
| **SkillUp** | GENERATIVE AI FOR BEGINNERS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/SkillUp.pdf) |

### 🥇 Awards & Recognition
* **🥈 2nd Prize** — Hackathon (For **Sahayak** AI Health Chatbot)
* **🧩 Active Participant** — Multiple National Level Hackathons
* **🎉 Tech Fest Lead** — Organized and participated in college coding competitions
---

## 🌟 Motto  
<p align="center">
  ⭐ <i>Code today. Innovate tomorrow.</i>
</p>
